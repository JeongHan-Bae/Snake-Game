...
enum MoveResult { WALL, APPLE, SELF, VOID, WIN };
...
private:
    struct RapidSet {
        ...
        RapidSet& operator=(const RapidSet& other);

        ~RapidSet();

        bool add(const std::pair<int, int>& element);

        bool replace(const std::pair<int, int>& toRemove,
...
public:
    SnakyImpl(int initialX, int initialY, int size,
    ...
    SnakyImpl& operator=(const SnakyImpl& other);
